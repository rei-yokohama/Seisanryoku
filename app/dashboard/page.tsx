...
const services: {
  ...
  { icon: I("M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"), label: "請求", href: "/billing", description: "請求書の作成・管理", permissionKey: "billing", accent: "hover:border-amber-400" },
  ...
  { icon: I("M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-2.54a4.5 4.5 0 00-6.364-6.364L4.5 8.25l4.5 4.5a4.5 4.5 0 006.364 0l1.757-1.757"), label: "連携", href: "/settings/webhooks", description: "Discord・Slack・Chatwork", accent: "hover:border-violet-400" },
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      try {
        ...
        if (!companyCode) { setPermissions(DEFAULT_PERMISSIONS); setLoading(false); return; }
      ...
      } catch { setPermissions(DEFAULT_PERMISSIONS); } finally { setLoading(false); }
    });
    return () => unsub();
