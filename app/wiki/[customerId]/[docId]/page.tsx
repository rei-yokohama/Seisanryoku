...
export default function WikiDocPage() {
  ...
  const params = useParams<{ customerId: string; docId: string }>();
  ...
  const doSaveNow = async (opts?: { force?: boolean }) => {
    ...
    try {
      ...
      payload.scopeType = dealId ? "DEAL" : "CUSTOMER";
      payload.scopeId = dealId || customerId;
      payload.shareEnabled = shareEnabled;
  ...
  const load = async (u: User) => {
    ...
    if (!nextDealId && d.scopeType === "DEAL" && d.scopeId) {
      nextDealId = String(d.scopeId || "");
    }
    ...
    // 案件候補の取得
    if (prof.companyCode) {
      const dealByCompany = await getDocs(query(collection(db, "deals"), where("companyCode", "==", prof.companyCode), where("customerId", "==", customerId)));
      const dealItems = dealByCompany.docs
        .map((x) => ({ id: x.id, ...x.data() } as Deal))
        .sort((a, b) => (a.title || "").localeCompare(b.title || ""));
      setDeals(dealItems);

      if (nextDealId && !dealItems.find((dd) => dd.id === nextDealId)) {
        nextDealId = "";
      }
    } else {
      setDeals([]);
  ...
  const canDelete = useMemo(() => !!user, [user]);
    ...
    try {
      ...
      await setDoc(
        doc(db, "wikiDocs", docId),
        {
          ...
          customerId,
          dealId: dealId || null,
          scopeType: dealId ? "DEAL" : "CUSTOMER",
          scopeId: dealId || customerId,
        },
        { merge: true }
      );
    } catch (e) {
  ...
  const toggleShare = async () => {
    ...
    if (!next && !confirm("共有リンクを無効にしますか？")) return;
    try {
      const effectiveCompanyCode = (profile.companyCode || docCompanyCode || "").trim();
      if (!effectiveCompanyCode) return;
      await setDoc(doc(db, "wikiDocs", docId), { shareEnabled: next, updatedAt: Timestamp.now() }, { merge: true });
      setShareEnabled(next);
      if (next) {
        const url = `${window.location.origin}/share/wiki/${docId}`;
        await navigator.clipboard.writeText(url);
        setShareCopied(true);
        setTimeout(() => setShareCopied(false), 3000);
      }
    } catch (e: any) {
      setError(e?.message || "共有設定の変更に失敗しました");
    }
  };
  ...
  return (
    ...
    >
      <div className="w-full">
        {error ? <div className="mb-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-700">{error}</div> : null}
        {!error && savedAt ? (
          ...
          <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
            ...
            <div className="sticky top-0 z-10 border-b border-slate-200 bg-white">
              <div className="flex flex-wrap items-center gap-3 px-4 py-3">
                <div className="flex-1 min-w-[240px]">
                  ...
                  <div className="mt-1 flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <span>顧客: <span className="font-bold text-slate-700">{customerName || "-"}</span></span>
                    {dealTitle && (
                      <>
                        <span className="text-slate-300">|</span>
                        <span>案件: <span className="font-bold text-slate-700">{dealTitle}</span></span>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  {deals.length > 0 && (
                    <select
                      ...
                      className="max-w-[240px] rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700"
                      title="案件"
                    >
                  ...
                  <Link href={`/wiki/${customerId}`} className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-50">
                    ← 一覧
                  </Link>
                  <button
                  ...
                  {shareEnabled ? (
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="rounded-md bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700 hover:bg-emerald-100 border border-emerald-200"
                      >
                        {shareCopied ? "コピー済み!" : "共有リンクをコピー"}
                      </button>
                      <button
                        onClick={toggleShare}
                        className="rounded-md bg-slate-50 px-2 py-1.5 text-xs font-bold text-slate-500 hover:bg-red-50 hover:text-red-600 border border-slate-200"
                      >
                        共有OFF
                      </button>
                    </div>
                    ...
                    <button
                      onClick={toggleShare}
                      disabled={!user}
                      className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-50"
                      title="共有リンクを発行（URLを知っている人は誰でも閲覧可能）"
                    >
                      共有リンク発行
                    </button>
                  )}
              ...
              <div className="flex flex-wrap items-center gap-1 px-4 pb-3">
                <button onClick={() => exec("undo")} className="rounded-md border border-slate-200 bg-white px-2 py-1 text-sm font-bold text-slate-700 hover:bg-slate-50" title="元に戻す">↶</button>
                <button onClick={() => exec("redo")} className="rounded-md border border-slate-200 bg-white px-2 py-1 text-sm font-bold text-slate-700 hover:bg-slate-50" title="やり直し">↷</button>
                <div className="mx-1 h-6 w-px bg-slate-200" />
                <button onClick={() => exec("bold")} className="rounded-md border border-slate-200 bg-white px-2 py-1 text-sm font-extrabold text-slate-800 hover:bg-slate-50" title="太字">B</button>
                <button onClick={() => exec("italic")} className="rounded-md border border-slate-200 bg-white px-2 py-1 text-sm font-bold italic text-slate-800 hover:bg-slate-50" title="斜体">I</button>
                <button onClick={() => exec("underline")} className="rounded-md border border-slate-200 bg-white px-2 py-1 text-sm font-bold underline text-slate-800 hover:bg-slate-50" title="下線">U</button>
                <div className="mx-1 h-6 w-px bg-slate-200" />
                <select
                  onChange={(e) => {
                ...
                <button onClick={() => exec("insertUnorderedList")} className="rounded-md border border-slate-200 bg-white px-2 py-1 text-sm font-bold text-slate-700 hover:bg-slate-50" title="箇条書き">••</button>
                <button onClick={() => exec("insertOrderedList")} className="rounded-md border border-slate-200 bg-white px-2 py-1 text-sm font-bold text-slate-700 hover:bg-slate-50" title="番号付きリスト">1.</button>
                <button
                  onClick={() => {
                ...
                <button onClick={() => exec("removeFormat")} className="rounded-md border border-slate-200 bg-white px-2 py-1 text-sm font-bold text-slate-700 hover:bg-slate-50" title="装飾を解除">Tx</button>
              </div>
            </div>
