...
const RELEASES: ReleaseEntry[] = [
  {
    date: "2026-03-01",
    title: "プロパティ設定3ページ化 / 課題編集の紐付け変更 / ブランドUI刷新",
    summary:
      "プロパティ設定を一覧・詳細・編集の3ページ構成に分割し、課題編集から案件・顧客の紐付けを変更可能に。グローバルメニューのロゴを明朝体＋雷アイコンに刷新しました。",
    added: [
      "プロパティ詳細ページ（/settings/properties/{id}）：読み取り専用表示・削除ゾーン（カスタムのみ）",
      "プロパティ編集ページ（/settings/properties/{id}/edit）：名前変更・選択肢の追加/削除をまとめて保存",
      "課題一覧（/issue）に「更新日」列を追加",
      "課題編集（/issue/{id}/edit）に案件・顧客の紐付け変更（検索ドロップダウン付き、未設定も可）",
      "グローバルメニューに雷SVGロゴマークを追加（折りたたみ時はマークのみ表示）",
    ],
    changed: [
      "プロパティ設定（/settings/properties）を一覧表示に簡素化（インライン編集/削除を詳細・編集ページへ移動）",
      "プロパティ一覧にシステムプロパティのバッジと説明文、選択肢のチップ表示を追加",
      "システムプロパティの名前を定義と自動同期（「カテゴリ」→「課題カテゴリ」等）",
      "サービス名「生産力」のフォントを明朝体（Shippori Mincho B1）に変更",
      "グローバルメニューをデフォルトで開いた状態に変更",
    ],
  },
  {
    date: "2026-01-24",
